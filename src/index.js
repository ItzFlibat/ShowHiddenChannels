...
class MissingZeresDummy {
  ...
  getSettingsPanel() {
    ...
    const buttonClicker = document.createElement("f");
      ...
      const plugin = (Plugin, Library) => {
        ...
        return class ShowHiddenChannels extends Plugin {
          ...
          Patch() {
            ...
            Patcher.instead(
              ...
              (instance, [fetchConfig], res) => {
                if (
                  ChannelStore.getChannel(fetchConfig.channelId)?.isHidden?.()
                ) {
                  return;
                }
          ...
          getHiddenChannelRecord(categories, guildId) {
            ...
            for (const category of categories) {
              ...
              if (category.hiddenChannelIds) {
                category.hiddenChannelIds = category.hiddenChannelIds.filter(
                  (v) => filteredChannelRecords.some(([id]) => id == v)
                );
              }

              if (category.shownChannelIds) {
                category.shownChannelIds = category.shownChannelIds.filter(
                  (v) => filteredChannelRecords.some(([id]) => id == v)
                );
              }
            }
          ...
          getSettingsPanel() {
            const {
              IconSwitchWrapper,
            } = require("./components/IconSwitchWrapper");
